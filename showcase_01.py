...
Original file is located at
    https://colab.research.google.com/drive/1YzcSBldf-n2iU9K5Gaim3ga68a9nHCNB
"""

from google.colab import drive
drive.mount('/content/drive')

!ls /content/drive/MyDrive/colab_backup

!pip install dash==2.18.1 dash-bootstrap-components geopandas shapely pyproj --quiet
...
LOCAL_GEOJSON = os.path.join(BASE_DIR, "NSW_LGA_2025.geojson")

# ✅ Google Drive 直连下载链接（你刚才给的）
REMOTE_GEOJSON_URL = "https://drive.google.com/uc?export=download&id=1ghuKn9kdYmtmfn9rHN-xiIws3Z444Ga5"

def load_geojson():
    """优先用本地 data/ 里的 geojson，没有就去 Google Drive 拉。"""
...
def norm(name):
    ...
    s = str(name).upper()
    s = re.sub(r"\([^)]*\)", "", s)
    s = re.sub(r"COUNCIL OF THE CITY OF|THE CITY OF|CITY OF|COUNCIL OF|MUNICIPAL COUNCIL|REGIONAL COUNCIL|SHIRE COUNCIL|CITY COUNCIL", "", s)
    s = re.sub(r"[^A-Z ]", " ", s)
    s = re.sub(r"\b(OF|THE|AND|LOCAL|AREA|REGION)\b", " ", s)
    s = re.sub(r"\s+", " ", s).strip()
    return s

# ========== 1️⃣ 收集每年的 council 名称 ==========
council_years = {}
for f in files:
    year = re.search(r"(\d{4}-\d{2})", f).group(1)
    df = pd.read_excel(os.path.join(BASE, f), header=2)
    councils = df["Council Name"].dropna().astype(str).tolist()
    councils = [norm(c) for c in councils if "TOTAL" not in c.upper() and "DIVISION" not in c.upper()]
    council_years[year] = set(councils)
    print(f"✅ {year}: {len(councils)} LGAs")
...
for f in files:
    year = re.search(r"(\d{4}-\d{2})", f).group(1)
    df = pd.read_excel(os.path.join(BASE, f), header=2)
    metric_years[year] = [c.strip().replace("\n", " ") for c in df.columns if "Council" not in c]

# ========== 3️⃣ 比较 LGA 名称 ==========
years = sorted(council_years.keys())
diffs = []
for i in range(len(years) - 1):
    y1, y2 = years[i], years[i + 1]
    new = council_years[y2] - council_years[y1]
    lost = council_years[y1] - council_years[y2]
    if new or lost:
        diffs.append((y1, y2, new, lost))
        print(f"\n🔹 {y2} 有但 {y1} 没有:")
        print(sorted(list(new)))
        print(f"🔸 {y1} 有但 {y2} 没有:")
        print(sorted(list(lost)))

# ========== 4️⃣ 比较列名变化 ==========
col_diffs = defaultdict(list)
for i in range(len(years) - 1):
    y1, y2 = years[i], years[i + 1]
    c1, c2 = set(metric_years[y1]), set(metric_years[y2])
    added = c2 - c1
    removed = c1 - c2
    if added or removed:
        col_diffs[(y1, y2)].append((added, removed))
        print(f"\n📊 列名变化 {y1} → {y2}:")
        print("新增列:", list(added)[:10])
        print("删除列:", list(removed)[:10])